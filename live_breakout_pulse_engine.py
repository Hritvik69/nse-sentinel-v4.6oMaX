...
• Curated universe — ~500 most-liquid NSE stocks (avoids API blocks).
...
# ─────────────────────────────────────────────────────────────────────
# CURATED LIQUID UNIVERSE  (~500 most-traded NSE stocks)
# Covers Nifty50, Nifty100, Nifty200 + high-beta mid-caps
# ─────────────────────────────────────────────────────────────────────
_LIQUID_500 = [
    # ── Nifty 50 core ────────────────────────────────────────────────
...
# CONSTANTS
# ─────────────────────────────────────────────────────────────────────
_MAX_WORKERS    = 4          # conservative — avoids yfinance rate-limit
_REQUEST_DELAY  = 0.06       # seconds between downloads (per thread)
...
def _score_ticker(ticker_ns: str, cutoff: date | None) -> dict | None:
    """
    ...
    df = _download_live(ticker_ns, cutoff)
    if df is None:
        return None
...
def run_live_breakout_pulse(
    ...
    # ── Build ticker list (de-duped, .NS suffix) ───────────────────
    tickers_ns = [
        (t if t.endswith(".NS") else f"{t}.NS")
        for t in _LIQUID_500
    ]
    ...
    def _worker(ticker_ns: str) -> dict | None:
        return _score_ticker(ticker_ns, cutoff_date)

    with ThreadPoolExecutor(max_workers=_MAX_WORKERS) as ex:
        futs = {ex.submit(_worker, t): t for t in tickers_ns}
