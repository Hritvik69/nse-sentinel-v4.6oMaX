...
"""
#https://nse-sentinel-v46omax-8iqrsn365tvqmayubz3ywn.streamlit.app/
#.\.venv\Scripts\python.exe -m streamlit run app.py
...
def fetch_nse_tickers() -> list:
    ...
    # ── SOURCE 1: nse_tickers.txt committed in the repo (ALWAYS works, 2500+) ──
    # Instructions: commit the nse_tickers.txt file (provided) to your GitHub repo
    # next to app.py. That's all you need to do — this code reads it automatically.
    tickers: set[str] = set()
    try:
        import pathlib
        _f = pathlib.Path(__file__).parent / "nse_tickers.txt"
        if _f.exists():
            _syms = [s.strip() for s in _f.read_text().splitlines() if s.strip()]
            tickers.update(f"{s}.NS" for s in _syms)
    except Exception:
        pass

    # ── SOURCE 2: hardcoded baseline (always available, ~1019 unique) ─────────
    tickers.update(f"{s}.NS" for s in _NSE_MAINBOARD)

    # ── SOURCE 3: GitHub raw URLs (work on Streamlit Cloud, not on local proxy) ─
    if len(tickers) < 2000:
        _GITHUB_URLS = [
        ...
        for _url in _GITHUB_URLS:
            try:
                _r = requests.get(_url, timeout=10, headers={"User-Agent":"Mozilla/5.0"})
                if _r.status_code == 200 and len(_r.content) > 300:
                   for _line in _r.text.splitlines():
                        _s = _line.strip().split(",")[0].replace('"','').replace('.NS','').upper().strip()
                        if _s and 2 <= len(_s) <= 20 and not any(c in _s for c in ' /\".'):
                            tickers.add(f"{_s}.NS")
                if len(tickers) > 2500:
                    break
            except Exception:
    ...
    # ── SOURCE 4: NSE direct (works locally, blocked on Streamlit Cloud) ────────
    if len(tickers) < 1500:
        try:
            _s2 = requests.Session()
            _s2.headers.update({"User-Agent":"Mozilla/5.0","Referer":"https://www.nseindia.com/"})
            _s2.get("https://www.nseindia.com/", timeout=6)
            _r2 = _s2.get("https://archives.nseindia.com/content/equities/EQUITY_L.csv", timeout=12)
            if _r2.status_code == 200 and len(_r2.content) > 5000:
                _df2 = pd.read_csv(io.StringIO(_r2.text))
                _col = "SYMBOL" if "SYMBOL" in _df2.columns else _df2.columns[0]
                for _sym in _df2[_col].dropna().unique():
                    tickers.add(f"{str(_sym).strip()}.NS")
        except Exception:
            pass
    ...
    if len(result) < 500:
        ...
        result = sorted(f"{s}.NS" for s in _NSE_MAINBOARD)
    return result
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in as_completed(futures):
            ...
            status.markdown(
                ...
                f' &nbsp;·&nbsp; Found <b style="color:#00d4a8">{len(results)}</b> candidates'
                f' &nbsp;·&nbsp; Speed <b style="color:#8ab4d8">{rate:.1f}/s</b></div>',
                unsafe_allow_html=True)
            eta_box.markdown(
                f'<div class="status-line" style="justify-content:center">'
...
# ─────────────────────────────────────────────────────────────────────
if _SKLEARN_OK and not st.session_state.get("_startup_ml_fired"):
    st.session_state["_startup_ml_fired"] = True
    def _startup_bg_train():
        """Warm up the mode-specific ML engine. Never blocks the UI."""
        try:
